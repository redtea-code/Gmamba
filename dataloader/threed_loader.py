...
class MRI_classify(data.Dataset):
    ...
    def find_row(self, ID, current_datetime):
        ...
        if min < min_orig:
            ...
            print("找不到日期误差小于30天" + " 对应数据(匹配数据信息：ID:" + str(ID) + "||date：" + str(
                current_datetime) + ")！")
    ...
    def __getitem__(self, index):
        ...
        if self.import_table:
            ...
            batch['cate_x'] = torch.tensor(self.table_dict['cate_x'].iloc[date_index].values,
                                           dtype=torch.float32).unsqueeze(-1)
            batch['conti_x'] = torch.tensor(self.table_dict['conti_x'].iloc[date_index].values,
                                            dtype=torch.float32).unsqueeze(-1)
        # 从mri_path中获取name
...
if __name__ == "__main__":
    ...
    train_dataloader = form_dataloader('E:/ADNI_Dataset/MRI-PET',
