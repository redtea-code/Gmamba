...
def tile_weights(args, embeddings):
    ...
    if args.feature_extractor_dims[0] > embeddings.shape[0]:
        ...
        embeddings = torch.cat([embeddings, embeddings[:args.feature_extractor_dims[0] % embeddings.shape[0], :]],
                               dim=0)
...
def create_model(args, data_module=None):
    ...
    ### create models
    if args.model == 'fsnet':
        concrete_layer = ConcreteLayer(args, args.num_features, args.feature_extractor_dims[0], is_diet_layer=True,
        ...
        if args.model == 'dnn':
            ...
            if args.winit_initialisation == 'gcondnet':
                ...
                match args.winit_graph_connectivity_type:
                    case 'sparse-relative-distance':
                        list_of_edges = create_edges_graphs_sparse_relative_distance(data_module.X_train_raw,
                                                                                     ratio_diff_to_neighbor=0.05,
                                                                                     max_degree=25)
                    case 'random':
            ...
            if args.winit_initialisation == 'pca':  # PCA-based weight initialisation
                ...
                initial_weights = data_module.get_embedding_matrix('svd',
                                                                   embedding_size=args.feature_extractor_dims[0]).T
                ...
                initial_weights = data_module.get_embedding_matrix('nmf',
                                                                   embedding_size=args.feature_extractor_dims[0]).T
                initial_weights = tile_weights(args, initial_weights)
                ...
                initial_weights, _ = compute_WL_graph_embeddings(data_module.X_train_raw,
                                                                 embeddings_size=args.feature_extractor_dims[0])
                initial_weights = tile_weights(args, initial_weights)
        ...
        for i, dim in enumerate(args.diet_network_dims):
            if i == len(args.diet_network_dims) - 1:  # last layer
                ...
                if args.nonlinearity_weight_predictor == 'tanh':
                    layer = nn.Linear(prev_dimension, dim)
                    nn.init.uniform_(layer.weight, -0.01,
                                     0.01)  # same initialization as in from the DietNetwork original paper
                    layers.append(layer)
...
class GNNWeightPredictorNetwork(nn.Module):
    ...
    def forward(self):
        ...
        weights = (graph_embeddings - graph_embeddings.mean(dim=0)) / (
                graph_embeddings.std(dim=0) + 1e-6)  # add a small epsilon to avoid division by zero
...
class FirstLinearLayer(nn.Module):
    ...
    def __init__(self, args, layer_type, sparsity_type, initial_weights=None, alpha_interpolation=None, **kwargs):
        ...
        if layer_type == "diet_gnn":
            self.wpn = GNNWeightPredictorNetwork(args, kwargs['gnn_model'], kwargs['gnn_batch'],
                                                 kwargs['std_kaiming_normal'])

        # if the final weight matrix is interpolated between W_GNN and W_scratch
        self.weights = nn.Parameter(torch.zeros(args.feature_extractor_dims[0], args.num_features),
                                    requires_grad=True)
...
class ConcreteLayer(nn.Module):
    ...
    def __init__(self, args, input_dim, output_dim, is_diet_layer=False, wpn_embedding_matrix=None):
        ...
        if is_diet_layer:
            ...
            torch.nn.init.xavier_normal_(self.alphas,
                                         gain=1)  # Glorot normalization, following the original CAE implementation
    ...
    def get_temperature(self):
        ...
        if self.current_iteration >= self.anneal_iterations:
            ...
            return self.temp_start * (self.temp_end / self.temp_start) ** (
                    self.current_iteration / self.anneal_iterations)
...
class TrainingLightningModule(pl.LightningModule):
    ...
    def compute_loss(self, y_true, y_hat, x, x_hat, sparsity_weights):
        losses = {}
        losses['cross_entropy'] = F.cross_entropy(input=y_hat, target=y_true,
                                                  weight=torch.tensor(self.args.class_weights, device=self.device))
        losses['reconstruction'] = F.mse_loss(x_hat, x, reduction='mean') if self.decoder else torch.zeros(1,
                                                                                                           device=self.device)
    ...
    def validation_epoch_end(self, outputs):
        ...
        if 'graph_reconstruction' in outputs[0]['losses']:
            losses['graph_reconstruction'] = np.mean(
                [output['losses']['graph_reconstruction'].item() for output in outputs]),
    ...
    def configure_optimizers(self):
        ...
        if self.args.optimizer == 'adamw':
            optimizer = torch.optim.AdamW(params, lr=self.learning_rate, weight_decay=self.args.weight_decay,
                                          betas=[0.9, 0.98])
...
class DNN(TrainingLightningModule):
    ...
    def __init__(self, args, first_layer, second_layer=None, decoder=None, dim=None, layer=2):
        """
        ...
        encoder_first_layers, encoder_second_layers = create_linear_layers(
            args, dim,
            layer - 1)  # the -1 in (args.layers_for_hidden_representation - 1) is because we don't consider the first layer
