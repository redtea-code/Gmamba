...
def create_edges_graphs_sparse_relative_distance(X, ratio_diff_to_neighbor=0.05, max_degree=25):
    ...
    for feature_id in range(X.shape[1]):
        diff = ratio_diff_to_neighbor * (
                    np.percentile(X[:, feature_id], 95, interpolation='midpoint') - np.percentile(X[:, feature_id], 5,
                                                                                                  interpolation='midpoint'))

        if diff == 0:  # some features from 'lung' and other datasets have almost the same value, and the difference is 0
            diff = ratio_diff_to_neighbor * (X[:, feature_id].max() - X[:,
                                                                      feature_id].min()) + 1e-5  # add 1e-5 because some features have the same value and diff is 0
...
if __name__ == '__main__':
    x = torch.rand(4,30)
    y = compute_WL_graph_embeddings(x,embeddings_size=5)