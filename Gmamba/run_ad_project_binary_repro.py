...
This runner keeps the checked-in ``Graph_mamba_`` classifier, uses the supplied
...
from graph_construct.cross_task_patch_graph_model import (
    CrossTaskPatchGraphClassifier,
    MriMriEncoderPatchGraphClassifier,
    MriPatchGraphClassifier,
    MriPetMriEncoderPatchGraphClassifier,
    MriPetPatchGraphClassifier,
)
...
class AblatedGraphMamba(nn.Module):
    """Remove one learned modality representation while keeping the backbone fixed."""

    def __init__(self, base_model: Graph_mamba_, ablation: str):
        super().__init__()
        ...
        self._hooks = []
        if ablation == "no_mri":
            self._hooks.append(self.base_model.mri_mamba.register_forward_hook(self._zero_output))
        elif ablation == "no_pet":
            self._hooks.append(self.base_model.pet_mamba.register_forward_hook(self._zero_output))
...
def prepare_clinical_features(
    table: pd.DataFrame, dataset: str, fit_indices: Iterable[int] | None = None
) -> dict[str, Any]:
    ...
    if dataset == "nacc" and "NACCMMSE" in feature_columns:
        feature_columns.remove("NACCMMSE")
        excluded_columns.append("NACCMMSE")
    ...
    for column in feature_columns:
        ...
        if has_text:
            ...
            if dataset == "nacc":
                values[np.isin(values, [-4, 88, 97, 98, 99])] = np.nan
            encoded_numeric[column] = values

    if numeric_columns:
        numeric_matrix = np.column_stack([encoded_numeric[column] for column in numeric_columns])
        fit = np.arange(len(table), dtype=np.int64) if fit_indices is None else np.asarray(list(fit_indices), dtype=np.int64)
        if fit.size == 0 or np.any(fit < 0) or np.any(fit >= len(table)):
            raise ValueError("fit_indices must contain valid table row indices")
        fit_matrix = numeric_matrix[fit]
        mean = np.nanmean(fit_matrix, axis=0)
        std = np.nanstd(fit_matrix, axis=0)
        mean = np.nan_to_num(mean, nan=0.0)
        std = np.nan_to_num(std, nan=1.0)
        std[std == 0] = 1.0
        numeric_matrix = ((np.nan_to_num(numeric_matrix, nan=mean) - mean) / std).astype(np.float32)
    else:
    ...
    return {
        ...
        "numeric_mean": mean.tolist() if numeric_columns else [],
        "numeric_std": std.tolist() if numeric_columns else [],
        "normalization_fit_indices": fit.tolist() if numeric_columns else [],
    }
...
def discover_records(
    ...
    metadata = {
        ...
        "excluded_columns": clinical["excluded_columns"],
        "numeric_mean": clinical["numeric_mean"],
        "numeric_std": clinical["numeric_std"],
        "normalization_fit_indices": clinical["normalization_fit_indices"],
...
def build_image_condition(
    ...
    if feature_mode == "mri_only":
        return MriPatchGraphClassifier.build_image_condition(
            mri=moved["image"],
            pet_gen=moved.get("pet"),
        )
    if feature_mode == "mri_pet_latent_concat":
        return CrossTaskPatchGraphClassifier.build_image_condition(
            mri=moved["image"],
            pet_gen=moved["pet"],
            z_rec=moved["z_rec"],
    ...
    if feature_mode == "mri_pet_mri_encoder":
        return MriPetMriEncoderPatchGraphClassifier.build_image_condition(
            mri=moved["image"],
            pet_gen=moved["pet"],
            z_gen_mri=moved["z_gen_mri"],
        )
    if feature_mode == "mri_mri_encoder":
        return MriMriEncoderPatchGraphClassifier.build_image_condition(
            mri=moved["image"],
            pet_gen=None,
            z_gen_mri=moved["z_gen_mri"],
        )
    ...
    for batch in loader:
        ...
        with torch.autocast(device_type="cuda", dtype=torch.float16, enabled=autocast_enabled):
            logits = model(
                moved["cate_x"],
                moved["conti_x"],
                build_image_condition(moved, feature_mode),
            )
...
def train(args: argparse.Namespace) -> None:
    ...
    extra_encoder_cnn = bool(args.extra_encoder_cnn)
    ...
    if args.batch_size != 1:
        raise ValueError("Graph_mamba_ reproduction currently requires --batch-size 1")
    ...
    if feature_mode == "mri_only":
        model_config.update(
            {
                "classifier": "MriPatchGraphClassifier",
                "source_in_channels": {"mri": 1},
                "source_projected_channels": 1,
                "target_size": [20, 24, 20],
                "patch_size": list(patch_size),
                "connectivity": 6,
                "graph_sources": list(MriPatchGraphClassifier.source_names),
            }
        )
        model = MriPatchGraphClassifier(
            ...
            connectivity=6,
            extra_encoder_cnn=extra_encoder_cnn,
        ).to(device)
    elif feature_mode == "mri_pet_latent_concat":
        source_channels = {
            "mri": 1,
            "pet_gen": 1,
            "z_rec": LATENT_FEATURE_FILES["z_rec"][1],
            "z_gen_mri": LATENT_FEATURE_FILES["z_gen_mri"][1],
            "z_gen_pet": LATENT_FEATURE_FILES["z_gen_pet"][1],
        }
        model_config.update(
            {
                "classifier": "CrossTaskPatchGraphClassifier",
                "source_in_channels": source_channels,
                "source_projected_channels": 1,
                "target_size": [20, 24, 20],
                "patch_size": list(patch_size),
                "connectivity": 6,
                "graph_sources": list(CrossTaskPatchGraphClassifier.source_names),
            }
        )
        model = CrossTaskPatchGraphClassifier(
            categories=tuple(data_metadata["category_sizes"]),
            num_continuous=int(data_metadata["num_continuous"]),
            dim=args.dim,
            depth=args.depth,
    ...
    elif feature_mode == "mri_mri_encoder":
        source_channels = {"mri": 1, "z_gen_mri": LATENT_FEATURE_FILES["z_gen_mri"][1]}
        model_config.update(
            {
                "classifier": "MriMriEncoderPatchGraphClassifier",
                "source_in_channels": source_channels,
                "source_projected_channels": 1,
                "target_size": [20, 24, 20],
                "patch_size": list(patch_size),
                "connectivity": 6,
                "graph_sources": list(MriMriEncoderPatchGraphClassifier.source_names),
            }
        )
        model = MriMriEncoderPatchGraphClassifier(
            categories=tuple(data_metadata["category_sizes"]),
            num_continuous=int(data_metadata["num_continuous"]),
            dim=args.dim,
            depth=args.depth,
    ...
    elif feature_mode == "mri_pet_mri_encoder":
        source_channels = {"mri": 1, "pet_gen": 1, "z_gen_mri": LATENT_FEATURE_FILES["z_gen_mri"][1]}
        model_config.update(
            {
                "classifier": "MriPetMriEncoderPatchGraphClassifier",
                "source_in_channels": source_channels,
                "source_projected_channels": 1,
                "target_size": [20, 24, 20],
                "patch_size": list(patch_size),
                "connectivity": 6,
                "graph_sources": list(MriPetMriEncoderPatchGraphClassifier.source_names),
            }
        )
        model = MriPetMriEncoderPatchGraphClassifier(
            categories=tuple(data_metadata["category_sizes"]),
            num_continuous=int(data_metadata["num_continuous"]),
            dim=args.dim,
            depth=args.depth,
        ...
        if args.ablation != "full":
            raise ValueError("The graph_construct MRI+PET mode currently supports --ablation full only")
        model_config.update(
            {
                "classifier": "MriPetPatchGraphClassifier",
                "source_in_channels": {"mri": 1, "pet_gen": 1},
                "source_projected_channels": 1,
                "target_size": [20, 24, 20],
                "patch_size": list(patch_size),
                "connectivity": 6,
                "graph_sources": ["mri", "pet_gen"],
            }
        )
        model = MriPetPatchGraphClassifier(
            categories=tuple(data_metadata["category_sizes"]),
            num_continuous=int(data_metadata["num_continuous"]),
            dim=args.dim,
            depth=args.depth,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default="mri_pet",
        help="Use MRI+PET only or fuse MRI, PET, and the three saved generator latent maps.",
    )
    parser.add_argument(
        "--generated-pet-root",
